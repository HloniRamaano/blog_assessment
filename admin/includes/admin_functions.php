...
$sql_users = "SELECT * FROM users WHERE role IS NOT NULL";
$result = mysqli_query($conn, $sql_users);
if($result)
{
	$admin_users_num = mysqli_num_rows($result);
}

$sql_posts = "SELECT * FROM posts";
$result = mysqli_query($conn, $sql_posts);
if($result)
{
...
function createTopic($request_values){
	...
	if (count($errors) == 0) {
		...
		{
			$_SESSION['message'] = "Topic not created!";
		}
